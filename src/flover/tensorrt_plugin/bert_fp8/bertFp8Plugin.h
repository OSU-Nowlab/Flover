...
#include "src/flover/models/bert_fp8/BertFP8.h"
...
struct CublasCtx {
    ...
    std::unique_ptr<flover::cublasFP8MMWrapper> mCublasWrapper;
    std::unique_ptr<flover::cublasAlgoMap>      mCublasAlgoMap;
    std::mutex                                             mCublasWrapperMutex;

    CublasCtx(flover::Allocator<flover::AllocatorType::CUDA>* allocator)
    {
        cudaStreamCreate(&mStream);
        cublasCreate(&mCublasHandle);
        cublasLtCreate(&mCublasLtHandle);
        cublasSetStream(mCublasHandle, mStream);
        mCublasAlgoMap.reset(new flover::cublasAlgoMap("gemm_config.in", ""));
        mCublasWrapper.reset(new flover::cublasFP8MMWrapper(
...
private:
    ...
    flover::NcclParam mTensorPara;
    flover::NcclParam mPipelinePara;
    ...
    std::unique_ptr<flover::Allocator<flover::AllocatorType::CUDA>> mAllocator;
    ...
    std::shared_ptr<flover::BertFP8Weight<fp8_t, bf16_t>> mBertWeights;
    std::unique_ptr<flover::BertFP8<fp8_t, bf16_t>>       mBertModel;
};
