...
#include "src/flover/utils/mpi_utils.h"
...
void initialize(int* argc, char*** argv)
{
    ...
    initThread(argc, argv, THREAD_MULTIPLE, &provided);
#endif
}
...
template<typename T>
void debugPrint(T* buff, int size, int tp_rank, int pp_rank, char* prefix){
    T*   hBuf     = new T[size];
    cudaD2Hcpy(hBuf, buff, size);
    // int s1 = size / 4, e1 = (size>s1+1000) ? (s1 + 1000) : size;
    int s1 = 0, e1 = size;
    if(tp_rank==0 && pp_rank==0){
        FILE *fp = fopen("/home/yao.877/parallel_inference/z_test/nccl/test00.txt", "a");
        fprintf(fp, "%s\n", prefix);
        for(int i=s1;i<e1;i++){
            fprintf(fp, "%f ", hBuf[i]);
        }
        ...
        fclose(fp);
        fp = NULL;
    }else if(tp_rank==0 && pp_rank==1){
        FILE *fp = fopen("/home/yao.877/parallel_inference/z_test/nccl/test01.txt", "a");
        fprintf(fp, "%s\n", prefix);
        for(int i=s1;i<e1;i++){
            fprintf(fp, "%f ", hBuf[i]);
        }
        ...
        fclose(fp);
        fp = NULL;
    }else if(tp_rank==1 && pp_rank==0){
        FILE *fp = fopen("/home/yao.877/parallel_inference/z_test/nccl/test10.txt", "a");
        fprintf(fp, "%s\n", prefix);
        for(int i=s1;i<e1;i++){
            fprintf(fp, "%f ", hBuf[i]);
        }
        ...
        fclose(fp);
        fp = NULL;
    }else if(tp_rank==1 && pp_rank==1){
        FILE *fp = fopen("/home/yao.877/parallel_inference/z_test/nccl/test11.txt", "a");
        fprintf(fp, "%s\n", prefix);
        for(int i=s1;i<e1;i++){
            fprintf(fp, "%f ", hBuf[i]);
        }
...
template void
debugPrint(__nv_bfloat16* buff, int size, int tp_rank, int pp_rank, char* prefix);
template void
debugPrint(float* buff, int size, int tp_rank, int pp_rank, char* prefix);
template void
debugPrint(int* buff, int size, int tp_rank, int pp_rank, char* prefix);
template void
debugPrint(__half* buff, int size, int tp_rank, int pp_rank, char* prefix);
template void
debugPrint(bool* buff, int size, int tp_rank, int pp_rank, char* prefix);
}  // namespace mpi
}  // namespace flover
