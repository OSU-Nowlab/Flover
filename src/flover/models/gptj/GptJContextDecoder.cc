        ...
        for (int l = 0; l < num_layer_; l++) {
            ...
            if (isFirstLayerParallelId(l) && pipeline_para_.rank_ != 0 && pipeline_para_.world_size_ > 1) {
                ...
                ftNcclRecv(layer_input + data_size * tensor_para_.rank_,
                           ...
                           pipeline_para_,
                           stream_);
                // T* printdebug = layer_input;
                // char* prefix1 = "before gather";
                // mpi::debugPrint<T>(printdebug, (int)(batch_size * seq_len * hidden_units_), tensor_para_.rank_, pipeline_para_.rank_, prefix1);
                if (tensor_para_.world_size_ > 1) {
                    ftNcclAllGather(layer_input, layer_input, data_size, tensor_para_.rank_, tensor_para_, stream_);
                    // printdebug = layer_input;
                    // char* prefix2 = "after gather";
                    // mpi::debugPrint<T>(printdebug, (int)(batch_size * seq_len * hidden_units_), tensor_para_.rank_, pipeline_para_.rank_, prefix2);
                }
            }
            // if(l==1) mpi::debugPrint(layer_input, batch_size * seq_len * hidden_units_, 0, 0, "");
            ...
            // mpi::debugPrint(self_attn_output_, h_token_num * hidden_units_, 0, 0, "");
            ...
            if (is_final == false) {
                ...
                invokeAddBiasAttentionFfnResidual(layer_output,
                                                  ...
                                                  stream_);
                sync_check_cuda_error();
                // if(l==0) mpi::debugPrint(decoder_layer_output_, batch_size * seq_len * hidden_units_, 0, 0, "");
