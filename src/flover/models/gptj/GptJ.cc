...
#include "src/flover/models/gptj/GptJ.h"
                      ...
                      const GptJWeight<T>*                           gpt_weights)
{
    ...
    if (has_prefix_prompt_ || has_prefix_soft_prompt_ || max_input_length > 1) {
        ...
        // mpi::debugPrint(context_decoder_output_buf_, batch_size * beam_width*max_input_length*hidden_units_, 0, 0, "");
        // T* printdebug = decoder_output_buf_;
        // mpi::debugPrint<T>(printdebug, (int)(batch_size * beam_width * hidden_units_), tensor_para_.rank_, pipeline_para_.rank_);
        sync_check_cuda_error();
    ...
    if (vocab_size_ == vocab_size_padded_) {
        ...
        // cudaMemcpy(pre_print_var_, gpt_weights->post_decoder_embedding.kernel, sizeof(T) * hidden_units_ * vocab_size_padded_, cudaMemcpyDeviceToDevice);
        // T* printdebug = pre_print_var_;
        // mpi::debugPrint<T>(printdebug, (int)(hidden_units_ * vocab_size_padded_), tensor_para_.rank_, pipeline_para_.rank_);
    }
    else {
        ...
        *generation_should_stop_   = true;
        ...
        for (uint ite = 0; ite < iteration_num; ++ite) {
            ...
            if (!(max_input_length > 1 && step == max_input_length)) {
                ...
                // cudaStreamSynchronize(stream_);
                // T* printdebug = decoder_input_buf_;
                // mpi::debugPrint<T>(printdebug, (int)(batch_size * beam_width * hidden_units_), tensor_para_.rank_, pipeline_para_.rank_);
                
                // printf("start gpt decoder forward...\n");
            ...
            if (pipeline_para_.rank_ == pipeline_para_.world_size_ - 1) {
                ...
                if (tensor_para_.world_size_ == 1) {
                    ...
                    // cudaStreamSynchronize(stream_);
                    // pre_print_var_ = (T*)(allocator_->reMalloc(pre_print_var_, sizeof(T) * local_vocab_size * hidden_units_, true));
                    // cudaMemcpy(pre_print_var_, padded_embedding_kernel_ptr_+ tensor_para_.rank_ * local_vocab_size * hidden_units_, sizeof(T) * local_vocab_size * hidden_units_, cudaMemcpyDeviceToDevice);
                    // T* printdebug = pre_print_var_;
                    // mpi::debugPrint<T>(printdebug, (int)(local_vocab_size * hidden_units_), tensor_para_.rank_, pipeline_para_.rank_);
                    cublas_wrapper_->Gemm(CUBLAS_OP_T,
                                          CUBLAS_OP_N,
                                          local_vocab_size,  // n
                                          local_batch_size * beam_width,
                    ...
                    // float* printdebug = nccl_logits_buf_ + vocab_size_units_offset;
                    // mpi::debugPrint<float>(printdebug, (int)(local_batch_size * beam_width * local_vocab_size), tensor_para_.rank_, pipeline_para_.rank_);
                    // printdebug = nccl_logits_buf_ + vocab_size_units_offset + local_batch_size * beam_width * local_vocab_size;
                    // mpi::debugPrint<float>(printdebug, (int)(local_batch_size * beam_width * local_vocab_size), tensor_para_.rank_, pipeline_para_.rank_);  
                    // printf("%d %d %d\n", tensor_para_.rank_, vocab_size_units_offset, local_batch_size * beam_width * local_vocab_size);
                    // float* printdebug = nccl_logits_buf_;
                    // mpi::debugPrint<float>(printdebug, (int)(batch_size*beam_width * vocab_size_padded_), tensor_para_.rank_, pipeline_para_.rank_);
                ...
                // printf("%d %d %d %d\n", step, max_input_length, ite, tmp_local_batch_size);
                ...
                // cudaStreamSynchronize(stream_);
                // pre_print_var_ = (T*)(allocator_->reMalloc(pre_print_var_, sizeof(T) * vocab_size_, true));
                // cudaMemcpy(pre_print_var_, padded_embedding_bias_ptr_, sizeof(T) * vocab_size_, cudaMemcpyDeviceToDevice);
                // int* printdebug = cache_indirections_;
                // mpi::debugPrint<int>(printdebug, (int)(2), tensor_para_.rank_, pipeline_para_.rank_);
