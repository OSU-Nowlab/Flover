...
#include "src/flover/th_op/decoder/DecoderOp.h"

namespace th = torch;
namespace torch_ext {

floverDecoder::floverDecoder(th::Tensor self_layernorm_gamma,
...
floverDecoder::~floverDecoder()
{
    delete ftdecoder;
}

std::vector<th::Tensor> floverDecoder::forward(int64_t    step,
...
std::vector<th::Tensor> floverDecoder::get_pickle_info() const
{
...
static auto floverDecoderTHS =
#ifdef LEGACY_THS
    torch::jit::class_<torch_ext::floverDecoder>("floverDecoder")
#else
    torch::jit::class_<torch_ext::floverDecoder>("flover", "Decoder")
#endif
        .def(torch::jit::init<th::Tensor,
                              ...
                              int64_t,
                              int64_t>())
        .def("forward", &torch_ext::floverDecoder::forward)
        .def_pickle(
            [](const c10::intrusive_ptr<torch_ext::floverDecoder>& self) -> std::vector<th::Tensor> {
                return self->get_pickle_info();
            },
            [](std::vector<th::Tensor> state) -> c10::intrusive_ptr<torch_ext::floverDecoder> {
                ...
                return c10::make_intrusive<torch_ext::floverDecoder>(state[0],
                                                                                state[1],
                                                                                state[2],
                                                                                state[3],
                                                                                state[4],
