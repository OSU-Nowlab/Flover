...
#include "src/flover/th_op/bart/BartDecoderOp.h"

namespace th = torch;
namespace torch_ext {

floverBartDecoder::floverBartDecoder(th::Tensor  self_layernorm_gamma,
...
floverBartDecoder::~floverBartDecoder()
{
    delete ftdecoder;
}

std::vector<th::Tensor> floverBartDecoder::forward(int64_t    step,
...
#ifdef LEGACY_THS
    torch::jit::class_<torch_ext::floverBartDecoder>("floverBartDecoder")
#else
    torch::jit::class_<torch_ext::floverBartDecoder>("flover", "BartDecoder")
#endif
        .def(torch::jit::init<th::Tensor,
                              ...
                              std::string,
                              std::string>())
        .def("forward", &torch_ext::floverBartDecoder::forward);
