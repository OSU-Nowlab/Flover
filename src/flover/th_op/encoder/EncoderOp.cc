...
#include "src/flover/th_op/encoder/EncoderOp.h"

namespace th = torch;
namespace torch_ext {

floverEncoder::floverEncoder(th::Tensor pre_attr_layernorm_gamma,
...
floverEncoder::~floverEncoder()
{
    delete ftencoder;
}

th::Tensor floverEncoder::forward(th::Tensor input, th::Tensor sequence_lengths)
{
...
std::vector<th::Tensor> floverEncoder::get_pickle_info() const
{
...
static auto floverEncoderTHS =
#ifdef LEGACY_THS
    torch::jit::class_<torch_ext::floverEncoder>("floverEncoder")
#else
    torch::jit::class_<torch_ext::floverEncoder>("flover", "Encoder")
#endif
        .def(torch::jit::init<th::Tensor,
                              ...
                              bool,
                              double>())
        .def("forward", &torch_ext::floverEncoder::forward)
        .def_pickle(
            [](const c10::intrusive_ptr<torch_ext::floverEncoder>& self) -> std::vector<th::Tensor> {
                return self->get_pickle_info();
            },
            [](std::vector<th::Tensor> state) -> c10::intrusive_ptr<torch_ext::floverEncoder> {
                ...
                return c10::make_intrusive<torch_ext::floverEncoder>(state[0],
                                                                                state[1],
                                                                                state[2],
                                                                                state[3],
                                                                                state[4],
