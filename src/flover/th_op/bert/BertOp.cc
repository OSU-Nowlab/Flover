...
#include "src/flover/th_op/bert/BertOp.h"

namespace th = torch;
namespace torch_ext {

floverBert::floverBert(th::Tensor q_kernel,
                                             th::Tensor q_bias,
                                             th::Tensor k_kernel,
                                             th::Tensor k_bias,
                                             th::Tensor v_kernel,
...
floverBert::~floverBert()
{
    delete ftbert;
}

th::Tensor floverBert::forward(th::Tensor input, th::Tensor sequence_lengths)
{
...
std::vector<th::Tensor> floverBert::get_pickle_info() const
{
...
static auto floverBertTHS =
#ifdef LEGACY_THS
    torch::jit::class_<torch_ext::floverBert>("floverBert")
#else
    torch::jit::class_<torch_ext::floverBert>("flover", "Bert")
#endif
        .def(torch::jit::init<th::Tensor,
                              ...
                              int64_t,
                              int64_t>())
        .def("forward", &torch_ext::floverBert::forward)
        .def_pickle(
            [](const c10::intrusive_ptr<torch_ext::floverBert>& self) -> std::vector<th::Tensor> {
                return self->get_pickle_info();
            },
            [](std::vector<th::Tensor> state) -> c10::intrusive_ptr<torch_ext::floverBert> {
                ...
                return c10::make_intrusive<torch_ext::floverBert>(state[0],
                                                                             state[1],
                                                                             state[2],
                                                                             state[3],
                                                                             state[4],
