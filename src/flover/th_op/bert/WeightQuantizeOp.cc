...
#include "src/flover/kernels/quantize_weight.h"
...
Tensor weight_quantize(Tensor weight, Tensor quant_max, bool sparse)
{
    ...
    if (sparse) {
        ...
        flover::invokeQuantizeWeight(tmp_out, weight_, quant_max_, n, k, format, stream);
        compressInt8Matrix(transform_out, tmp_out, n, k, stream);
        return output;
    }
#endif
    int format = use_ORDER_COL32_2R_4R4 ? 1 : 2;
    flover::invokeQuantizeWeight(transform_out, weight_, quant_max_, n, k, format, stream);
    return output;
}
...
static auto weight_quantize =
    torch::RegisterOperators("flover::weight_quantize", &torch_ext::weight_quantize);
