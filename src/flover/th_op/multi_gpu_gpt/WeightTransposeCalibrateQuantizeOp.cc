...
#include "src/flover/th_op/multi_gpu_gpt/WeightTransposeCalibrateQuantizeOp.h"
#include "src/flover/utils/convert_data_type.h"
...
std::vector<Tensor> weight_transpose_calibrate_quantize(Tensor weight)
{
    ...
    if (weight.device() == torch::kCUDA) {
        ...
        flover::invokeLdnCalibrateWeightPerChannel(scale_out, weight_, k, n, stream);
        flover::invokeLdnTransposeQuantizeWeightPerChannel(
...
static auto weight_transpose_calibrate_quantize = torch::RegisterOperators(
    "flover::weight_transpose_calibrate_quantize", &torch_ext::weight_transpose_calibrate_quantize);
