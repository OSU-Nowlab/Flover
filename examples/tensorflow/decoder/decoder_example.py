...
layer of TensorFlow by the decoder of flover.

This sample code builds a decoding model by TensorFlow, and user can replace
the decoder transformer layer of model by the decoder of flover. 
...
and the new model is like:
    embedding-lookup -> position encoding -> flover decoder -> beam search
...
which will run the both TensorFlow decoder and flover in one model, 
