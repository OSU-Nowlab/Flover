...
class FTBartEncoderWeight(object):
    ...
    def load_from_model(self, model):  
        ...
        Weight loading order: PyTorch tensor order should conform to src/flover/th_op/BartEncoderOp.h:floverBartEncoder. For per-layer weights, the tensor is a stack of the weight across all layers.
        '''
...
class FTBartEncoder(nn.Module):
    def __init__(self, encoder_weight_list, lib_path, head_num, head_size, inter_size, d_model, is_remove_padding,
        ...
        try:
            self.encoder = torch.classes.flover.BartEncoder(*encoder_weight_list, head_num, head_size, inter_size, d_model,
            ...
            self.encoder = torch.classes.floverBartEncoder(*encoder_weight_list, head_num, head_size, inter_size, d_model,
                                                                    is_remove_padding, num_layer, num_bucket, max_distance, sparse, q_scaling, tensor_para_size, pipeline_para_size,
                                                                    bart_with_bias, mbart, position_embedding_type, activation_type, layernorm_type)
