...
class FTBartDecodingWeight(object):
    ...
    def load_from_model(self, model):
        ...
        Weight loading order: PyTorch tensor order should conform to src/flover/th_op/BartDecodingOp.h:floverBartDecoding. For per-layer weights, the tensor is a stack of the weight across all layers.
        '''
...
class FTBartDecoding(nn.Module):
    def __init__(self, decoding_weight_list, lib_path, head_num, head_size, inter_size,
        ...
        try:
            self.decoding = torch.classes.flover.BartDecoding(head_num, head_size, inter_size, mem_d_model, d_model, num_layer,
            ...
            self.decoding = torch.classes.floverBartDecoding(head_num, head_size, inter_size, mem_d_model, d_model, num_layer,
                                                                       vocab_size, num_bucket, max_distance, q_scaling, start_id, end_id,
                                                                       tensor_para_size, pipeline_para_size, bart_with_bias, mbart,
                                                                       position_embedding_type, activation_type, layernorm_type, *decoding_weight_list)
