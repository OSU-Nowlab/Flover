...
class FTDecoder(nn.Module):
    def __init__(self, head_num, head_size, mem_hidden_dim, layer_num, weights, args):
        ...
        try:
            self.dec_layer = torch.classes.flover.Decoder(*weights.w, head_num, head_size, head_num * head_size * 4, layer_num, mem_hidden_dim)
        except:
            # legacy ths for 20.03 image
            self.dec_layer = torch.classes.floverDecoder(*weights.w, head_num, head_size, head_num * head_size * 4, layer_num, mem_hidden_dim)
